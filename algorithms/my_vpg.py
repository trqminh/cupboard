...
class VanillaPolicyGradient(object):
    ...
    def train(self, env):
        ...
        if is_continuous:
            ...
            log_std = torch.tensor(-0.5*np.ones(out_layer_dim), dtype=torch.float32, requires_grad=True, device=self.device)
        else:
            out_layer_dim = env.action_space.n
            output_activation = nn.Identity

        policy = mlp(sizes = [obs_dim] + self.hidden_sizes + [out_layer_dim], output_activation=output_activation).to(self.device)
        baseline_model = mlp([obs_dim] + self.hidden_sizes + [1]).to(self.device)
        ...
        optimizer_mse = optim.Adam(baseline_model.parameters(), lr=self.lr)
        ...
        for epoch in range(self.n_epochs):
            ...
            while True:
                ...
                logit = policy(torch.from_numpy(obs).to(dtype=torch.float, device=self.device))
